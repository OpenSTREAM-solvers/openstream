classdef (Abstract) AbstractFilm < Solvers.AbstractField
        ...
        function ment = MENT(absfilm,zIdx)
            ...
            switch model.ENTRAINMENT
                ...
                case InputEnums.ENTRAINMENT.OKAWA2003
                    ...
                    thresholds = [];
                    coefs = [4.79e-4 0.111];
                    [k, n] = absfilm.OKAWAMENT(entnum, thresholds, coefs);
                    ment = (k*rhof).*entnum.*(rhof/rhog).^n;               % [kg/m^2/s] Entrainment mass flux
                ...
                case InputEnums.ENTRAINMENT.OKAWA2004
                    ...
                    thresholds = [0.0675 0.295];    % Model thresholds
                    coefs = [3.1e-2 2.3 1.6e-3 1.2 6.8e-4 0.5];     % Model coefficients
                    [k, n] = absfilm.OKAWAMENT(entnum, thresholds, coefs);
                    ment = (k*rhof).*entnum.^n;               % [kg/m^2/s] Entrainment mass flux
                    ment(absfilm.RE(zIdx)<=Refc) = 0;                                   % Set to 0 below critical film Reynolds
            end
    ...
    methods(Access = private)
        ...
        function [k, n] = OKAWAMENT(film, entnum, thresholds, coefs)
            %OKAWAMENT Private method to calculate the entrainment rate using
            %a generic framework following Okawa et al. (2004)
            %https://doi.org/10.1016/j.nucengdes.2011.01.033
            %returns k and n

            t = [-1e-5 thresholds 1e5];
            k = zeros(size(entnum));
            n = zeros(size(entnum));

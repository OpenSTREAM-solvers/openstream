classdef SolverIntegrationTest < matlab.unittest.TestCase
    ...
    % boundary-condition input sets. The two-fluid, three-field, and
    % four-field solvers reuse the converged mixture-solver solution.
    %
    ...
    properties (ClassSetupParameter)
        ...
        %   MixtureCase        Index of the mixture solver used for initialization. A value of zero indicates that no existing mixture solver is supplied.
    ...
    methods (TestClassSetup)
        ...
        function constructAndSolveTestCase(testCase, case_option)
            ...
            %   5. Constructs and solves every solver object.
            %
            % The solved objects are retained in testCase.solvers and
            % reused by all test methods.
            ...
            testCase.outputFolder = fullfile(testCase.testFolder,'outputs',string(case_option.Number));
            ...
            % Import the OpenSTREAM packages.
            import Inputs.*
            % import Solvers.*
            ...
            % % Preallocate the shared test properties.
            % 
            % numberOfCases = numel(cases);
            % testCase.testNumber = zeros(1,numberOfCases);
            % testCase.inputSets = cell(1,numberOfCases);
            % testCase.solvers = cell(1,numberOfCases);

            % Construct and solve test case.

                % Store the test identifier.
                testCase.testNumber = case_option.Number;
                ...
                inputSet = InputSet( ...
                    ...
                    modelID               = case_option.ModelID, ...
                    optionsFilePath       = fullfile(testCase.inputFolder,'options.inp'), ...
                    optionsID             = case_option.OptionsID, ...
                    geometryFilePath      = fullfile(testCase.inputFolder,'geom.inp'), ...
                    geometryID            = case_option.GeometryID, ...
                    bcFilePath            = fullfile(testCase.inputFolder,case_option.BoundaryCondition + ".inp"), ...
                ...
                % if case_option.MixtureCase == 0
                %     % The mixture solver does not require an existing mixture solution.
                %     testCase.solver= case_option.Constructor(testCase.inputSet);
                % else
                %     % Retrieve the solved mixture object used to initialize the advanced solver.
                %     mixtureSolver = testCase.solver{case_option.MixtureCase};
                %     % Construct the advanced solver using the common mixture solution.
                %     testCase.solver = case_option.Constructor(testCase.inputSet,mixtureSolver);
                % end
    ...
    methods (Test)
        ...
        function mixtureBoundaryConditions(testCase)
            ...
            suppliedInputSet = testCase.inputSet;
        ...
        function saveLoadRoundTrip(testCase)
            % SAVELOADROUNDTRIP
            %
            % Verify that each solver object:
            %
            %   1. Can be saved to a MAT-file.
            %   2. Can be loaded from that MAT-file.
            %   3. Retains the solver class.
            %   4. Retains the solver state.
            %
            % The solver is stored using the variable name "solver".

            for solverCell = testCase.solver

classdef SolverIntegrationTest < matlab.unittest.TestCase
    ...
    properties (SetAccess = private)
        ...
        inputSets         % Input sets independently retained by the test class.
        solvers           % OpenSTREAM solver objects.
    ...
    methods (TestClassSetup)
        ...
        function constructAndSolveTestCases(testCase)
            % CONSTRUCTANDSOLVETESTCASES
            %
            ...
            testCase.outputFolder = fullfile(testCase.testFolder,'outputs');
            ...
            % Construct and solve every test case.

            for caseIndex = 1:numberOfCases

                % Store the test identifier.
                testCase.testNumber(caseIndex) = cases(caseIndex).Number;
                ...
                inputSet = InputSet( ...
                    ...
                    modelID               = cases(caseIndex).ModelID, ...
                    optionsFilePath       = fullfile(testCase.inputFolder,'options.inp'), ...
                    optionsID             = cases(caseIndex).OptionsID, ...
                    geometryFilePath      = fullfile(testCase.inputFolder,'geom.inp'), ...
                    geometryID            = cases(caseIndex).GeometryID, ...
                    bcFilePath            = fullfile(testCase.inputFolder,cases(caseIndex).BoundaryCondition + ".inp"), ...
                    sessionParentDir      = testCase.outputFolder, ...
                    overwriteSessionFiles = true, ...
                    LOGMODE = 'BOTH');
                testCase.inputSets{caseIndex} = inputSet;
                ...
                if cases(caseIndex).MixtureCase == 0
                    % The mixture solver does not require an existing mixture solution.
                    testCase.solvers{caseIndex} = cases(caseIndex).Constructor(testCase.inputSets{caseIndex});
                else
                    % Retrieve the solved mixture object used to initialize the advanced solver.
                    mixtureSolver = testCase.solvers{cases(caseIndex).MixtureCase};
                    % Construct the advanced solver using the common mixture solution.
                    testCase.solvers{caseIndex} = cases(caseIndex).Constructor(testCase.inputSets{caseIndex},mixtureSolver);
                end

                % Solve the flow fields.
                testCase.solvers{caseIndex}.solve();
            end
    ...
    methods (Test)
        ...
        function solverStates(testCase)
            % SOLVERSTATES
            %
            % Verify that every solver reaches the expected converged state.

            for caseIndex = 1:numel(testCase.solvers)

                % Retrieve the calculated solver object.
                results = testCase.solvers{caseIndex};
        ...
        function mixtureBoundaryConditions(testCase)
            ...
            % Locate the mixture-solver case.
            mixtureCaseIndex = find(testCase.testNumber == 1,1);

            testCase.assertNotEmpty( ...
                mixtureCaseIndex, ...
                "The mixture-solver test case was not found.");
            ...
            suppliedInputSet = testCase.inputSets{mixtureCaseIndex};
        ...
        function saveLoadRoundTrip(testCase)
            ...
            for solverCell = testCase.solvers

                % Retrieve the original solver object.
                original = solverCell{1};
